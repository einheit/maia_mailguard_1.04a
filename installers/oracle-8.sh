...
echo 
echo "this script is written for oracle 8, using a mysql DB" 
...
# basic dependencies - 
yum install -y curl wget make gcc sudo net-tools less which rsync libdb-devel
...
# smarty missing from ol8? fall back to manual install
tar -C /tmp -xzvf files/smarty-3.1.34.tar.gz
mv /tmp/smarty-3.1.34/lib /usr/share/php/Smarty
